import React, { useState } from 'react';
import { Link, useHistory } from 'react-router-dom';
import axios from 'axios';
import { postUserRegister } from '../Api/Requests';

function Register(props) {
	const [email, setEmail] = useState('');
	const [fullName, setFullName] = useState('');
	const [password, setPassword] = useState('');
	const [confirmPassword, setConfirmPassword] = useState('');
	const [contactNumber, setContactNumber] = useState('');
	const history = useHistory();

	async function registerUser(e) {
		e.preventDefault();
		if (email && fullName && contactNumber && password && confirmPassword) {
			if (password === confirmPassword) {
				try {
					const body = { email, fullName, password, contactNumber };
					await postUserRegister(body);

					// Redirect to login
					let path = `/login`;
					history.push(path);
				} catch (error) {
					alert('FaceLog ID already registered');
				}
			} else {
				alert('Passwords do not match');
			}
		} else {
			alert('Mandatory fields');
		}
	}

	return (
		<>
			<div className="relative p-1">
				<div className="relative bg-gradient-to-r from-gray-800 to-blue-900 md:pt-32 pb-32 pt-12 rounded-2xl">
					{/* Card */}
					<div className="flex flex-wrap">
						<div className="w-full px-10">
							<div className="relative flex flex-col min-w-0 break-words mb-6 xl:mb-0">
								<div className="flex-auto px-4">
									<div className="flex flex-wrap">
										<div className=" w-full pr-4 max-w-full flex-grow flex-auto">
											<span className="flex justify-center text-center text-white  mr-0 whitespace-no-wrap text-5xl font-bold px-0">
												FaceLog
											</span>
										</div>
									</div>

									<div class="min-h-screen block mt-4 justify-center sm:px-6 lg:px-8">
										<div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
											<div class="bg-white pb-8 pt-4 px-4 shadow sm:rounded-lg sm:px-10">
												<div class="sm:mx-auto sm:w-full sm:max-w-md pb-6 pt-1 text-center">
													<h2 class="text-2xl font-poppins text-blue-800">
														Create a FaceLog account
													</h2>
												</div>
												<form class="space-y-6">
													<div>
														<label
															for="email"
															class="block text-sm font-poppins text-gray-700"
														>
															FaceLog ID
														</label>
														<div class="mt-1">
															<input
																id="email"
																name="email"
																type="email"
																autocomplete="email"
																required
																placeholder='eg: krishrajpal@facelog'
																onChange={(e) => setEmail(e.target.value)}
																class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm"
															/>
														</div>
													</div>
													<div>
														<label
															for="fname"
															class="block text-sm font-poppins text-gray-700"
														>
															Full Name
														</label>
														<div class="mt-1">
															<input
																id="fname"
																name="fname"
																type="text"
																required
																onChange={(e) => setFullName(e.target.value)}
																class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm"
															/>
														</div>
													</div>

													<div>
														<label
															for="contact"
															class="block text-sm font-poppins text-gray-700"
														>
															Contact Number
														</label>
														<div class="mt-1">
															<input
																id="contact"
																name="contact"
																type="text"
																autocomplete="email"
																required
																onChange={(e) => setContactNumber(e.target.value)}
																class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none sm:text-sm"
															/>
														</div>
													</div>
													<div>
														<label
															for="password"
															class="block text-sm font-poppins text-gray-700"
														>
															Password
														</label>
														<div class="mt-1">
															<input
																id="password"
																name="password"
																type="password"
																autocomplete="current-password"
																required
																onChange={(e) => setPassword(e.target.value)}
																class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-cyan-500 sm:text-sm"
															/>
														</div>
													</div>

													<div>
														<label
															for="password"
															class="block text-sm font-poppins text-gray-700"
														>
															Confirm Password
														</label>
														<div class="mt-1">
															<input
																id="password"
																name="password"
																type="password"
																autocomplete="current-password"
																required
																onChange={(e) => setConfirmPassword(e.target.value)}
																class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-cyan-500 sm:text-sm"
															/>
														</div>
													</div>
													<div>
														<Link to="/login">
															<button
																type="submit"
																class="w-full flex justify-center py-4 px-4 border border-transparent rounded-md shadow-sm text-base font-poppins text-white bg-blue-800 focus:outline-none  hover:bg-blue-500"
																onClick={(e) => registerUser(e)}
															>
																Sign up
															</button>
														</Link>
													</div>
												</form>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</>
	);
}
export default Register;
