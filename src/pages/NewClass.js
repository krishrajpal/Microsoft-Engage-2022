...
function NewClass(props) {
	...
	const modelImages = (event) => {
		...
		let message = `${images.length} valid image(s) imported`;
		alert(message);
								...
								<div className="flex-auto p-4">
									<div className="flex flex-wrap">
										<div className=" w-full pr-4 max-w-full flex-grow flex-auto">
											<span className="flex center text-blue-800 mr-0 whitespace-no-wrap text-3xl font-bold p-4 px-0">
												FaceLog New Course
											</span>
										</div>
									</div>
									<div class="flex flex-col px-80 mt-6 mb-4">
										<label class="flex flex-col justify-center">
											<span class="text-gray-600 font-semibold">Course ID:Course Name </span>
											<input
												type="text"
												onChange={(e) => setCourseName(e.target.value)}
												class="form-input mt-2 block focus:border-purple-800 rounded"
												placeholder="eg: ME221:Fluid Mechanics"
											/>
										</label>
										...
										<label class="flex flex-col mt-6">
											...
											<span class="text-gray-600 font-light">
												Upload images of each student, and the names of the file should be the student roll numbers.
											</span>
										</label>
										...
										<Link to='/home'>
										<button
											class="mt-6 flex flex-col items-center px-2 py-4 rounded text-white bg-blue-800 hover:bg-blue-500 focus:border-white"
											onClick={(e) => createClass(e)}
										>
											<span class="font-semibold justift-center">Save Details ➞</span>
										</button>
										</Link>
									</div>
								</div>
